// ==========================================
// PAQUETE VIP - Tus 15 con UBM
// ==========================================
import { getUserByPhone, sendMessage } from '../../utils/utils.js';
import db from '../../database.js';

export const PAQUETE_VIP = {
    nombre: 'Paquete VIP',
    emoji: '💎',
    imagen: 'https://drive.google.com/uc?export=view&id=17XwDYT5vimGSuITLA4ipnCwjTLPyeONu',
    descripcion: `📝 *¿Qué incluye el Paquete VIP?*
...
export async function handleVip(sock, from, conversationState) {
    const userId = from.split('@')[0];
    
    try {
        // Enviar imagen del paquete
        await sock.sendMessage(from, {
            image: { url: PAQUETE_VIP.imagen },
            caption: `${PAQUETE_VIP.emoji} *${PAQUETE_VIP.nombre}* 🌟`
        });
        
        // Enviar descripción
        await sendMessage(sock, from, PAQUETE_VIP.descripcion);
        
        // Preguntar si quiere más información
        await sendMessage(sock, from, `🤔 ¿Te gustaría recibir más información sobre el *${PAQUETE_VIP.nombre}*? Escribe *sí* o *no*:`);
        
        // Actualizar estado
        conversationState[from] = {
            step: 'TUS15_INTERES',
            data: { paquete: PAQUETE_VIP.nombre }
        };
        
    } catch (error) {
        console.error('❌ Error enviando información del Paquete VIP:', error);
        await sendMessage(sock, from, '⚠️ Hubo un error al enviar la información. Por favor, intenta nuevamente.');
        delete conversationState[from];
    }
}

export async function handleInteresVip(sock, from, respuesta, conversationState) {
    const userId = from.split('@')[0];
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                db.run(
                    ...
                    [user.id, PAQUETE_VIP.nombre, 'Interesado', new Date().toISOString()],
                    (err) => {
                        if (err) {
                            console.error("❌ Error al guardar interacción:", err.message);
                            return reject(err);
                        }
                        console.log(`✅ Interacción registrada: ${PAQUETE_VIP.nombre}`);
                        resolve();
                    }
                );
            });
            ...
            await sendMessage(sock, from, `✅ ¡Excelente! Hemos registrado tu interés en *${PAQUETE_VIP.nombre}* 🌟.
...
😊 Si tienes más preguntas, no dudes en escribirnos. ¡Gracias por elegirnos!`);
            ...
            console.error('❌ Error guardando interés en Paquete VIP:', error);
