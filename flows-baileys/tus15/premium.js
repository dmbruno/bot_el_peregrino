// ==========================================
// PAQUETE PREMIUM - Tus 15 con UBM
// ==========================================
import { getUserByPhone, sendMessage } from '../../utils/utils.js';
import db from '../../database.js';

export const PAQUETE_PREMIUM = {
    nombre: 'Paquete Premium',
    emoji: '⭐',
    imagen: 'https://drive.google.com/uc?export=view&id=1bwfyyxmBShBOZy09FhDewxS5PupI1tEE',
    descripcion: `📝 *¿Qué incluye el Paquete Premium?*
...
export async function handlePremium(sock, from, conversationState) {
    const userId = from.split('@')[0];
    
    try {
        // Enviar imagen del paquete
        await sock.sendMessage(from, {
            image: { url: PAQUETE_PREMIUM.imagen },
            caption: `${PAQUETE_PREMIUM.emoji} *${PAQUETE_PREMIUM.nombre}* 🌟`
        });
        
        // Enviar descripción
        await sendMessage(sock, from, PAQUETE_PREMIUM.descripcion);
        
        // Preguntar si quiere más información
        await sendMessage(sock, from, `🤔 ¿Te gustaría recibir más información sobre el *${PAQUETE_PREMIUM.nombre}*? Escribe *sí* o *no*:`);
        
        // Actualizar estado
        conversationState[from] = {
            step: 'TUS15_INTERES',
            data: { paquete: PAQUETE_PREMIUM.nombre }
        };
        
    } catch (error) {
        console.error('❌ Error enviando información del Paquete Premium:', error);
        await sendMessage(sock, from, '⚠️ Hubo un error al enviar la información. Por favor, intenta nuevamente.');
        delete conversationState[from];
    }
}

export async function handleInteresPremium(sock, from, respuesta, conversationState) {
    const userId = from.split('@')[0];
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                db.run(
                    ...
                    [user.id, PAQUETE_PREMIUM.nombre, 'Interesado', new Date().toISOString()],
                    (err) => {
                        if (err) {
                            console.error("❌ Error al guardar interacción:", err.message);
                            return reject(err);
                        }
                        console.log(`✅ Interacción registrada: ${PAQUETE_PREMIUM.nombre}`);
                        resolve();
                    }
                );
            });
            ...
            await sendMessage(sock, from, `✅ ¡Excelente! Hemos registrado tu interés en *${PAQUETE_PREMIUM.nombre}* 🌟.
...
😊 Si tienes más preguntas, no dudes en escribirnos. ¡Gracias por elegirnos!`);
            ...
            console.error('❌ Error guardando interés en Paquete Premium:', error);
